...
<div class="events index">
	...
	$i = 0;
	foreach ($event_parents as $event):
		$class = null;
		if ($i++ % 2 == 0) {
			$class = ' class="altrow"';
		}
	...
	<tr<?php echo $class;?>>
		...
		<td><?php echo $event['EventParent']['start']; ?></td>
        
   		<td><?php echo $event['EventParent']['recur_end']; ?></td>
		...
		<td class="actions">
			<?php echo $this->Html->link(__('View', true), array('action' => 'view', $event['EventParent']['id'])); ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $event['EventParent']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
